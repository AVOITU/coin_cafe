...
// const resultsDto = window.RESULTSDTO ?? {};
// -----------------------------------------------------------------------

document.getElementById('year').textContent = new Date().getFullYear();
document.getElementById('globalRatingText').textContent = resultsDto.globalRating.toFixed(1) + ' / 5';
...
const gauge = new Chart(document.getElementById('gaugeChart'), {
    ...
    data: {
        labels: ['Note','Reste'],
        datasets: [{ data: [resultsDto.globalRating, 5 - resultsDto.globalRating] }]
    },
    options: {
        responsive: true,
        cutout: '70%',
...
const pieLabels = Object.keys(resultsDto.pieCounts).map(k=>`Note ${k}`);
const pieValues = Object.values(resultsDto.pieCounts);
...
new Chart(document.getElementById('lineChart'), {
    type: 'line',
    data: { labels: resultsDto.timeline.labels, datasets: [{ label: 'Moyenne', data: resultsDto.timeline.values, tension: .3, fill: false }]},
...
new Chart(document.getElementById('barQuestionChart'), {
    type: 'bar',
    data: { labels: resultsDto.byQuestion.labels, datasets: [{ label: 'Moyenne', data: resultsDto.byQuestion.values }]},
