 ...
 *
 * Lit la table `survey_categories` (par survey_id),
 * génère pour chaque label une question adaptée au score,
 * appelle l'API Responses (OpenAI) en JSON Mode (json_object) ou Structured Outputs (json_schema),
 * et renvoie un JSON exploitable côté front.
 */
...
$CONFIG = [
  ...
  'use_json_schema' => false, // true = stricter / sortie "parsed"

  // --- Intensité par seuil (modifiable facilement)
  'thresholds' => [
    'faible' => 4.20,
    'ciblee' => 3.80,
    'forte'  => 3.50,
  ...
  // --- MySQL
  'db' => [
    'dsn'  => 'mysql:host=127.0.0.1;dbname=tysta1713984_6an8nq;charset=utf8mb4',
    'user' => 'tysta1713984_6an8nq',
    'pass' => 'TyStand29@',
  ],
];
...
try {
  ...
  echo json_encode(['error' => 'Connexion MySQL échouée: '.$e->getMessage()], JSON_UNESCAPED_UNICODE);
  exit;
}
...
$sql = "SELECT label, value FROM survey_categories WHERE survey_id = :sid ORDER BY pos";
...
if (!$rows) {
  ...
  header('Content-Type: application/json; charset=utf-8');
  echo json_encode(['error' => "Aucune catégorie trouvée pour survey_id=$surveyId"], JSON_UNESCAPED_UNICODE);
  exit;
}
 ...
 * - JSON mode (json_object) : type = "json_object"
 * - Structured Outputs : type = "json_schema"
 */
function buildTextFormat(bool $useSchema): array {
  if (!$useSchema) {
    return [
      "format" => [
        "type" => "json_object" // <<< IMPORTANT : JSON Mode
      ]
    ];
  }
...
foreach ($rows as $r) {
  ...
  try {
    ...
    // petite pause si beaucoup de thèmes
    usleep(150000); // 150ms
  } catch (Throwable $e) {
    $results[] = [
      'label' => $label,
